		...
		go func() {
			...
			for {
				...
				case <-ctx.Done():
					...
					log.Infof("total duration=%v, total bytesReceived=%v, total packetsReceived=%v total throughput=%.2f Mb/s\n", time.Since(start), totalBytesReceived, totalPacketsReceived, mBitPerSecond)
					return
